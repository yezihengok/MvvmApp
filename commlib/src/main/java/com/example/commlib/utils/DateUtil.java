...
public class DateUtil {
    ...
    /**
     * 获取某一年各星期的始终时间
     * 实例：getWeekList(2016)，第52周(从2016-12-26至2017-01-01)  年份
     *
     * @param
     * @return
     */
    ...
    public static Date addMinutes(int minute) {
        return addDate(new Date(), 0, 0, 0, 0, minute, 0, 0);
    }
     ...
     * @return String 返回值为：{天, 时, 分, 秒}
     */
    public static long[] getDistanceTime(String str1, String str2,String type) {
     ...
     * @return long[] 返回值为：{天, 时, 分, 秒}
     */
    public static Long getDistanceDays(String str1, String str2, String type) throws Exception {
...
//        String[] start = times.split(":");
//        LocalDateTime s = LocalDateTime.of(0, 0, 0, 0, Integer.parseInt(start[0]), Integer.parseInt(start[1]),Integer.parseInt(start[2])*1000);
        String[] times = time.split(":");
        int m = Integer.valueOf(times[0]);
        int s = Integer.valueOf(times[1]);
        int S = Integer.valueOf(times[2]);
        return m * 60 * 1000 + s * 1000 + S * 10;
    }

    /**
     * 获取时间段
     *
     * @param startTime "00:04:33"   分：秒：毫秒
     * @param endTime   "00:06:21"
     * @return
     */
    @SuppressLint("NewApi")
    public static long timeToStamp(String startTime, String endTime) {
        String[] start = startTime.split(":");
        String[] end = endTime.split(":");
        LocalDateTime s = LocalDateTime.of(2019, 1, 1, 1, Integer.parseInt(start[0]), Integer.parseInt(start[1]), Integer.parseInt(start[2]) * 10000);
        LocalDateTime e = LocalDateTime.of(2019, 1, 1, 1, Integer.parseInt(end[0]), Integer.parseInt(end[1]), Integer.parseInt(end[2]) * 10000);
        long nanos = Duration.between(s, e).toMillis();
        return nanos;
    }
    ...
    public static String switchCreateTime(String createTime, String dateFormat) {
        ...
        try {
            ...
            //再转换成自己想要显示的格式
            formatStr2 = new SimpleDateFormat(dateFormat).format(datetime);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return formatStr2;
    ...
    /**
     * double类型时间转date时间（4.2 转 4年2月）
     *
     * @param date 需转换的时间
     * @return 转换后的时间格式
     */
//    public static String doubleToDate(String date) {
//        String year = null;
//        String month = null;
//        String time = null;
//
...
//            return "0岁";
//        }
//        boolean isDian = date.contains(".");
//        if (isDian) {
//            month = date.substring(date.length() - 1);
//            month = String.valueOf(Math.round(Integer.parseInt(month) * 1.2));
//            year = date.substring(0, date.length() - 2);
//            if (Integer.parseInt(year) > 0) {
//                time = year + "岁" + month + "个月";
//            } else {
//                time = month + "个月";
//            }
//        } else {
...
//        return time;
//    }
    public static String doubleToDate(String date, String yearUnit, String monthUnit) {
        String year;
        String month;
        String time;
        ...
        if (isDian) {
            ...
            if (Integer.parseInt(year) > 0) {
                if (Integer.parseInt(month) > 0) {
                    time = year + yearUnit + month + monthUnit;

                } else {
                    time = year + yearUnit;
                }
            } else {
                time = month + monthUnit;
            }
        } else {
            time = date + yearUnit;
        }
    ...
    public static String doubleToDate(String date, String yearUnit) {
        String year;
        String time;
    ...
    public static boolean isToday(String date) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS);
     ...
     * Date时间戳转换为时间
     */
    public static String stampToDate(Date date, String pattern) {
        String res;
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
        res = simpleDateFormat.format(date);
        return res;
     ...
     * @return
     */
    public static int checkMin(Date beginDate, Date endDate) {
        long between = (endDate.getTime() - beginDate.getTime()) / 1000;
        long min = (between / 60);
        return (int) min;
    }

    public static String getYMD(long cur) {
        DateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT_YY_MM_DD);
        return dateFormat.format(new Date(cur));
    }

    private static long lastCheckUpdateTime = 0L;
    private static final long UPDATE_MIN_INTERVAL = TimeUnit.HOURS.toMillis(1);
     ...
     * @return HH:mm:ss.SSS
     */
    public static String msecToTime2(int time) {
        String timeStr = null;
        int hour = 0;
        int minute = 0;
        int second = 0;
